...
def sql_fetch_posts(*parse)
  ...
  loop do
    query = "SELECT t.topic_id, t.discourse_id, t.topic_title,
      ...
      AND f.forum_id IN (45, 46, 47, 5, 129, 130, 128, 125, 118, 105, 15, 92, 90, 97, 120, 88)
...
def sql_fetch_users
  ...
  loop do
    count = 0
    query = "SELECT user_id, username_clean, username, user_regdate,
    ...
    if User.where('username = ?', dc_username).empty? then
      ...
      begin
        ...
        dc_user = User.create!(username: dc_username,
                               ...
                               created_at: Time.at(phpbb_user['user_regdate'] ? phpbb_user['user_regdate'].to_i : phpbb_user['user_lastvisit']))
